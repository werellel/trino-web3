...
public final class BitcoinTestnetChainAdapter implements ExecutableChainAdapter
{
    ...
    {
        return new EndpointIdentityProbe(new RemoteOperation("getblockchaininfo", List.of()), BitcoinTestnetChainAdapter::identity);
    }

    @Override
    public List<ChainSplit> planSplits(ChainScan scan, ChainSplitLimits limits)
    {
        return new BitcoinChainAdapter().planSplits(scan, limits);
    }

    @Override
    public ChainDataClient createDataClient(RemoteExecutionRuntime runtime)
    {
        return new io.trino.plugin.web3.utxo.UtxoChainDataClient(runtime, "bitcoin_testnet");
    }

    private static String identity(JsonNode response)
    {
        if (!response.path("chain").isTextual() || !response.path("chain").textValue().equals("test") || !response.path("subversion").asText().startsWith("/Satoshi:")) {
            throw new IllegalArgumentException("invalid Bitcoin testnet node identity");
        }
    ...
    {
        try (InputStream input = BitcoinTestnetChainAdapter.class.getResourceAsStream("bitcoin-chain.json")) {
            ChainDescriptor base = ChainDescriptorCodec.fromJson(input);
            return new ChainDescriptor(base.apiVersion(), "bitcoin_testnet", "bitcoin_testnet", base.adapterVersion(), base.tables());
        }
        catch (Exception e) { throw new IllegalStateException("failed to load Bitcoin testnet descriptor", e); }
    }
}
